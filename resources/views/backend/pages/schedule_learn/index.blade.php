            ...
            <div class="row pl-5">
                ...
                <div>
                    <form style="margin-left:0px" class="form-inline pt-4">
                        <input style="width: 200px;" name="name" class="border-success bg-white form-control mr-sm-2"
                            type="text" placeholder="Tìm Theo Tên Lớp" aria-label="Search">
                        <a>
                            <button style="width: 95px;" class="border-success btn btn-outline-success my-2 my-sm-0"
    ...
    <tbody>
        ...
        <tr>
            ...
            <td style="padding-left: 150px">
                ...
                @else
                @if(array_search($item->id, $get_schedule) === false)
                <button style="width: 97px;" data-toggle="modal" data-target="#exampleModal_{{$item->id}}" type="button"
                    class="border-primary btn btn-outline-primary">Xếp Lịch</button>
                @else
                @if($item->count_studen_count == 0 && $item->teacher_id == null)
                <button style="width: 97px;" data-toggle="modal" data-target="#exampleModal_edit_{{$item->id}}"
                    id="button_edit_{{$item->id}}" type="button" class="border-warning btn btn-outline-warning">Đổi
                    Lịch</button>
                @else
                <button style="width: 97px;" data-toggle="modal" data-target="#exampleModal_edit_{{$item->id}}"
                    id="button_edit_{{$item->id}}" type="button" class="border-primary btn btn-outline-primary">Xem
                    Lịch</button>
                @if($item->teacher_id != null)
                <button style="width: 120px;" data-toggle="modal" data-target="#exampleModal_chuyen_lich_{{$item->id}}"
                    id="button_chuyen_lich_{{$item->id}}" type="button"
                    class="border-primary btn btn-outline-primary">Chuyển Lịch</button>
                @endif
                @endif
                @endif
                @endif
...
<form id="form_edit_{{ $item->id }}" action="{{ route('schedule_learn.store') }}" method="post">
    @csrf
    <div class="modal fade" id="exampleModal_edit_{{$item->id}}" tabindex="-1" role="dialog"
        aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header m-auto">
...
<form id="btn_chuyen_lich_form_{{ $item->id }}" action="{{ route('schedule_learn.update',$item->id) }}" method="post">
    @csrf
    @method('PUT')
    <div class="modal fade" id="exampleModal_chuyen_lich_{{$item->id}}" tabindex="-1" role="dialog"
        aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header m-auto">
                ...
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-dismiss="modal">Thoát</button>
                    <button type="button" id="button_an_chuyen_lich_{{$item->id}}" class="btn btn-primary">Chuyển
                        Lịch</button>
                </div>
            </div>
        </div>
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                $.each(response, function(weekday, slot) {
                    $(`#${weekday}_${id} option[value="${slot}"]`).prop("selected",
                        "selected");
                });
            }
        });
    ...
    $("button[id^='button_chuyen_lich_']").click(function() {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                $(`#ngay_muon_chuyen_${id}`).html(
                    `<option value="0">Chọn Ngày</option>${html}`);
            }
        });
    });
});
...
$("button[id^='button_an_chuyen_lich_']").click(function() {
    ...
    if (Date.parse(datep) - Date.parse(new Date()) < 0) {
        ...
        // ht
        if ($(`select#ngay_muon_chuyen_${id}`).val() == 0 || $(`input#ngay_chuyen_sang_${id}`).val() == null ||
            $('input[name=slot]:checked').val() == undefined) {
...
$("input[id^='ngay_chuyen_sang_']").on('change', function() {
    ...
    if ($(`select#ngay_muon_chuyen_${id}`).val() == 0) {
        ...
        html = "";
        $.ajax({
            url: '/admin/schedule_learn/show/lich_trong/' + $(`select#ngay_muon_chuyen_${id}`).val() +
                '/' + $(`input#ngay_chuyen_sang_${id}`).val(),
            method: 'get',
            success: function(response) {
                ...
                if (response.length == 0) {
                    html +=
                        "<span id='erorr_slot' style='color:red;margin-left:17px'>Không Có Ca Nào Trống Trong Ngày Này</span>";
                } else {
                    $('#erorr_slot').css('display', 'none');
                    response.map(x => {
                        html += `
                ...
                $(`#show_slot_null_${id}`).html(
                    `<label class="col-12" for="exampleFormControlSelect1">Các Ca Trống Trong Ngày</label>${html}`
                );
            }
        });
    }
